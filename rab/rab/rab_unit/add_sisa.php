<?php
	...
	$id = $_GET['id'];
	$sql_material = mysqli_query($con, "SELECT *, tb_material.id_material as idm FROM tb_material left join tb_rab_unit on tb_material.id_material = tb_rab_unit.id_material where tb_rab_unit.id_material is null") or die (mysqli_error($con));
	...
	if ($rows == 0) {
		echo "<script>alert('Tidak Ada Data Material yang harus diberikan RAB'); window.location='data.php?id=$id';</script>";
	}
?>
						...
						<?php for ($i=0; $i <= $rows ; $i++) { 
							...
							<tr>
								<td><?php $i++; ?>
									...
									<input type="hidden" name="id_material-<?=$i?>" value="<?=$data_material['idm']?>">
