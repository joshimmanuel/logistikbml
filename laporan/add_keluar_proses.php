							...
							<tr>
								...
								<td>
									<select name="unit[]" class="form-control" required>
										...
										<?php
										$sql_supplier = mysqli_query($con, "SELECT * FROM tb_unit where lokasi_unit = '$lokasi_user'") or die (mysqli_error($con));
										while ($data_supplier = mysqli_fetch_array($sql_supplier)) {
											echo '<option value="'.$data_supplier['id_unit'].'">'.$data_supplier['nama_unit'].'</option>';
										}
										?>
									</select>
									<input type="hidden" name="lokasi[]" value="<?=$lokasi_user?>">
								</td>
							</tr>
						<?php
							}	
